...
void ReadBackBufferData(ID3D12Resource* inBuffer, void* outData, size_t inSize, size_t inOffset)
{
    if(inBuffer == nullptr)
    {
        Log::Error("[D3D12] Invalid buffer");
    ...
    uint8_t* mappedData = nullptr;
    const D3D12_RANGE range = {inOffset, inSize};
    inBuffer->Map(0, &range, reinterpret_cast<void**>(&mappedData));
    memcpy(outData, mappedData, inSize);
    inBuffer->Unmap(0, nullptr);
}
...
bool VariableRateShadingDx::CreateDepthStencilBuffer()
{
...
bool VariableRateShadingDx::CreateResources()
{
    ...
    // Create Texture SRVs and Samplers
    for(uint32_t i = 0; i < s_TexturesCount; ++i)
    {
        ...
        heapHandle.ptr += i * m_DeviceHandle->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
        ...
        heapHandle.ptr += i * m_DeviceHandle->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
...
bool VariableRateShadingDx::CreateQueryResultResources()
{
    ...
    m_TimestampQueryResult = CreateBufferHelper(m_DeviceHandle.Get()
        , sizeof(uint64_t) * TimeQueryCount
        , D3D12_RESOURCE_STATE_COPY_DEST
        , D3D12_HEAP_TYPE_READBACK
        , D3D12_RESOURCE_FLAG_NONE);

    m_PipelineStatisticsQueryResult = CreateBufferHelper(m_DeviceHandle.Get()
        , 8
        , D3D12_RESOURCE_STATE_COPY_DEST
        , D3D12_HEAP_TYPE_READBACK
        , D3D12_RESOURCE_FLAG_NONE);
